...
class ScrapperMarketAgriculture:
    ...
    init_urls = [
        ['Frutas y Hortalizas', '/ConsultaFrutasYHortalizas.aspx',
            '/ResultadosConsultaFechaFrutasYHortalizas.aspx'],
        ['Flores', '/ConsultaFlores.aspx?SubOpcion=5',
            '/ResultadosConsultaFechaFlores.aspx'],
        ['Aceites', '/ConsultaAceites.aspx?SubOpcion=8',
    ...
    def read_category(self, category, url, url_form):
        ...
        category_page = BeautifulSoup(
            category_page.content, features="html.parser")

        products = [(product.getText(), product['value'], )
                    for product in category_page.select_one('select#ddlProducto').find_all('option')]
        ...
        for product in products:
            ...
            if self.is_historic:
                for year in range(1999, 2019):
                    ...
                    if not self.gather_prices(payload, url_form, product_name, product_id):
                        next
            else:
    ...
    def gather_prices(self, payload, url_form, product_name, product_id):
        with indent(4):
        ...
        if response.status_code != 200:
            with indent(4):
                puts(colored.red(
                    "Error en la peticion HTTP: {}".format(str(response.text))))
            return False

        product_prices = BeautifulSoup(
        ...
        fields = ()
        if self.is_historic:
            fields = ('fecha', 'presentacion', 'origen', 'destino',
                      'precio_min', 'precio_max', 'precio_frec', 'obs')
        else:
            fields = ('presentacion', 'origen', 'destino',
                      'precio_min', 'precio_max', 'precio_frec', 'obs')
        counter_row = 0
